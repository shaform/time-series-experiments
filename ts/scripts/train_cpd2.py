...
class CPDRNN(object):
        ...
        if self.bidirectional:
            if self.predict_each:
                ...
                for i in range(X.shape[2]):
                    backward_y_hat_each = self.back_rnn(
                        backward_X[:, :, i:i + 1])
                    backward_y_hat.append(backward_y_hat_each.unsqueeze(-1))
                backward_y_hat = torch.cat(backward_y_hat, dim=-1)
            else:
                backward_y_hat = self.back_rnn(backward_X)
            b_loss = self.criteria(
                backward_y_hat.view(backward_y_hat.contiguous().size(0), -1),
                backward_y.contiguous().view(backward_y_hat.size(0), -1))
    ...
    def train(self,
        ...
        def gen_semi():
            while True:
                for k in semi_dataloader:
                    yield k.transpose(0, 1).view(
                        k.size(1), -1, dataloader.num_variables).transpose(
                            0, 1).contiguous()
        ...
        for epoch in trange(num_epochs):
            ...
            for i, X in enumerate(t):
                ...
                optim.zero_grad()
                loss = self.forward_step(X)
                if semi_iter:
                    X_semi = next(semi_iter)
                    loss_semi = self.forward_step(X_semi)
                    loss += loss_semi
...
def train(args):
    ...
    if 'yahoo' in args.data_path:
        ...
        dataloader = LabeledDataSet(
            ...
            shuffle=False)
    if args.semi_path:
        semi_dataloader = UnlabeledDataLoader(
            data_paths=args.semi_path,
            window_size=args.window_size,
            batch_size=args.batch_size * dataloader.num_variables,
            device=device)
    if args.test:
        ...
        if args.load_path:
            rnn = torch.load(os.path.join(args.load_path, 'model.best'))
        else:
            rnn = CPDRNN(
        ...
        rnn.train(
            ...
            semi_dataloader=semi_dataloader,
            lr=args.lr,
            beta1=args.beta1,
            beta2=args.beta2,
...
def parse_args():
    ...
    parser.add_argument('--semi-path', nargs='+')
    ...
    parser.add_argument('--valid-niter', type=int, default=200)
